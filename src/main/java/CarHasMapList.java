...
public class CarHasMapList implements CarMap{
    ...
    public void put(CarOwner key, Car value) {
       ...
       boolean bl = put(key,value,array);
        if(bl){
           size++;
        }
    }

    public boolean put(CarOwner key, Car value,Entry std[]) {
        int position = getElementPosition(key,std.length);
        Entry existedElement = std[position];
        ...
        if (existedElement == null){
            ...
            while (true){
                if (existedElement.key.equals(key)){
                    existedElement.next.value = value;
                    return false;
                }
    ...
    public boolean remove(CarOwner key) {
        int position = getElementPosition(key,array.length);
        Entry existedElement = array[position];
        if (existedElement!=null && existedElement.equals(key)){
            existedElement = existedElement.next;
            size--;
            return true;
        }else{
            while(existedElement!= null){
                ...
                if (nextElement.key.equals(null)){
                    return false;
                }
